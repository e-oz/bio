...
The visual environment, **Pelagic Orbit**, brings a bioluminescent ocean together with a ringed planet. Animated water, stars, plankton, and translucent manta rays sit behind an ordinary, scrollable HTML page. “Explore the scene” opens an immersive view; Escape or “Back to the page” returns to the biography.

In the immersive desktop view, moving the pointer pans the camera smoothly. Foreground rays and particles move at different depths. The biography uses a quieter response, and paused motion freezes the interaction as well as the autonomous animation.
...
| `assets/pelagic-orbit.jpg` | Desktop environment image, approximately 331 KiB |
| `assets/pelagic-orbit-mobile.jpg` | Smaller environment image, approximately 92 KiB |
| `assets/fonts/` | Self-hosted Instrument Serif and Manrope fonts, with their licenses |
| `docs/artwork.md` | Image-generation method and complete source prompt |
| `docs/text-review.md` | The 27 approved text corrections |
...
- Rendering stops while the document is hidden, while the visitor reads beyond the hero, or when motion is paused. The immersive view resumes it when appropriate.
...
The two email buttons use the existing obfuscated strings in `data-email`. JavaScript converts them into keyboard-focusable `mailto:` links on request. This is light obfuscation, not protection against a determined scraper. Without JavaScript, a readable obfuscated address is provided.
